...
class DEMAnalyzer(object):
    ...
    def bounds(self,temp,dem,**kwargs):
        ...
        if len(i_cool_lower[0]) > 0 and len(i_cool_upper[0]) > 0 and temp_new[i_cool_upper[0][0] - 1] <= np.max(temp_new):
        ...
        if len(i_hot_lower[0]) > 0 and len(i_hot_upper[0]) > 0 and temp_new[i_hot_lower[0][-1] + 1] >= np.max(temp_new):
